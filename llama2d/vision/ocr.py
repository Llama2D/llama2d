...
class ImageAnnotation:
    ...
    midpoint_normalized: Tuple[float, float] # the normalized midpoint between 0 - 1  (X,Y)
...
class ImageAnnotator(object):
    ...
    def __call__(self, path):
        ...
        whole_text_box_max = annotations[0].bounding_poly.vertices[2] # slice out entire body of text
        ...
        for text in annotations[1:]:
            ...
            annotations_normed.append(ImageAnnotation(
                text = text.description,
                midpoint = midpoint,
                midpoint_normalized = (midpoint[0]/max_width,
                                    midpoint[1]/max_height),
            ))

        annotations_normed = list(sorted(annotations_normed, key=lambda x:(x.midpoint_normalized[1],
