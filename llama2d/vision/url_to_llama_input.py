...
class Llama2dWebsiteFeatureExtractor(object):
    ...
    def __init__(self, model_path, separator_id=None, label_mask_id=-100, mask_out_body = True): # -100 is default
        self.tokenizer = LlamaTokenizer.from_pretrained(model_path)
        self.__annotator = ImageAnnotator()

        if not separator_id:
            self.__separator_id = self.tokenizer.unk_token_id # this should be kept at 0 for most uses, as it is a special token
        else:
    ...
    def __process(self, prompt, page, output):
        ...
        input_ids = (
            ...
            [self.__separator_id] + # seperating prompt with context
            self.tokenizer.convert_tokens_to_ids([j for i in image_tokens for j in i]) + 
            [self.__separator_id] + # seperating context with answer
