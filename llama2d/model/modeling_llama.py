...
class Llama2DAttention(nn.Module):
    ...
    def forward(
        ...
        if self.use_2d:
            assert pos_embeds is not None,"pos_embeds must be passed to Llama2DAttention when using 2D attention"
...
class Llama2DDecoderLayer(nn.Module):
    def __init__(self, config: Llama2DConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Llama2DAttention(config=config)
...
class Llama2DPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["Llama2DDecoderLayer"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, LambdaGate):
            module.lbd.data.zero_()
